...
impl<'a> ScalaJsInterfaceSkeleton<'a> {
    pub fn new(
        ...
        let encoded_name = generator.context.encode_name(&name.to_kebab_case());
    ...
    pub fn generate(&mut self) {
        ...
        uwriteln!(source, "@JSExportTopLevel(\"{}\")", self.encoded_name.js);
        uwriteln!(
            source,
            "object {} extends {} {{",
            self.name,
...
impl<'a> ScalaJsExportedResourceSkeleton<'a> {
    pub fn new(owner: &'a ScalaJsInterfaceSkeleton<'a>, resource_id: TypeId) -> Self {
        ...
        let prefixed_export_name =
            format!("{}_{}", owner.encoded_name.js, encoded_resource_name.js);

        uwriteln!(
            class_header,
            "@JSExportTopLevel(\"{}\")",
            prefixed_export_name
        );
