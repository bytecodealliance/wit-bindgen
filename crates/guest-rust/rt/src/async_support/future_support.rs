...
#[doc(hidden)]
pub type LoweredWrite = [CoreVal; 4];
...
pub struct FutureVtable<T> {
    ...
    pub lower_write: unsafe fn(value: T, dst: &mut LoweredWrite) -> Option<Cleanup>,
    ...
    /// exported functions.
    pub dealloc_lists: unsafe fn(dst: &LoweredWrite),

    /// A callback to lift a value of `T` from the canonical ABI representation
    /// provided.
    ///
    /// This is for lifting a value lowered as a `future.write` parameter.
    pub lift_write: unsafe fn(dst: &LoweredWrite) -> T,

    /// A callback to lift a value of `T` from the canonical ABI representation
    /// provided.
    ///
    /// This is for lifting a value lowered as a `future.read` result.
    pub lift_read: unsafe fn(dst: *mut u8) -> T,

    /// A wrapper function for the raw `future.write` intrinsic.
    ///
    /// This will pass the specified lowered payload to the `future.write`
    /// intrinsic.
    pub start_write: unsafe extern "C" fn(future: u32, lowered: &LoweredWrite) -> u32,
    ...
    T: 'static,
{
    ...
    type InProgress = (FutureWriter<T>, LoweredWrite, Option<Cleanup>);
    ...
    fn start((writer, value): Self::Start) -> (u32, Self::InProgress) {
        ...
        //
        // SAFETY: `lower_write` and `start_write` will handle allocation (if
        // needed), lowering, and union field access here according to the
        // payload type.  All we need to do is give it a `LoweredWrite`
        // reference to work with.
        let mut lowered = [CoreVal { uninitialized: () }; 4];
        let (code, cleanup) = unsafe {
            let cleanup = (writer.vtable.lower_write)(value, &mut lowered);
            (
                (writer.vtable.start_write)(writer.handle, &lowered),
                cleanup,
            )
        };
        rtdebug!("future.write({}) = {code:#x}", writer.handle);
        (code, (writer, lowered, cleanup))
    }
    ...
    fn in_progress_update(
        (mut writer, lowered, cleanup): Self::InProgress,
        code: u32,
    ) -> Result<Self::Result, Self::InProgress> {
        match ReturnCode::decode(code) {
            ReturnCode::Blocked => Err((writer, lowered, cleanup)),

            // The other end has dropped its end.
            //
            // The value was not received by the other end so `lowered` still
            // has all of its resources intact. Use `lift` to construct a new
            // instance of `T` which takes ownership of pointers and resources
            // and such.
            c @ ReturnCode::Dropped(0) | c @ ReturnCode::Cancelled(0) => {
                // SAFETY: TODO
                let value = unsafe { (writer.vtable.lift_write)(&lowered) };
            ...
            //
            // Here we need to clean up our allocations. `lowered` exclusively
            // owns all of the value being sent and we notably need to cleanup
            // the transitive list allocations present. Use `dealloc_lists` for
            // that (effectively a post-return lookalike).
            ReturnCode::Completed(1) | ReturnCode::Dropped(1) | ReturnCode::Cancelled(1) => {
                // A value was written, so no need to write the default value.
                writer.should_write_default_value = false;

                // SAFETY: TODO
                unsafe {
                    (writer.vtable.dealloc_lists)(&lowered);
                }
    ...
    fn in_progress_waitable((writer, ..): &Self::InProgress) -> u32 {
        writer.handle
    }

    fn in_progress_cancel((writer, ..): &Self::InProgress) -> u32 {
    ...
    fn in_progress_update(
        ...
        match ReturnCode::decode(code) {
            ...
            ReturnCode::Completed(1) | ReturnCode::Dropped(1) | ReturnCode::Cancelled(1) => {
                ...
                // pass here.
                let value = unsafe { (reader.vtable.lift_read)(ptr) };
