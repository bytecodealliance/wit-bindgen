...
/// called `export_my_world`. The naming here is `export_{name}!(...)` where
/// `{name}` is based on the name of the `world` that's being bound here. The
/// `export_my_world!` macro is given a component that implements the export
/// `trait`s and then it will itself generate all necessary `#[no_mangle]`
/// functions to implement the ABI required.
///
...
/// `std::cell` module.
///
/// ## Exports: The `export_{name}!` macro
///
...
/// `export_{name}!` macro.
///
/// Each call to `generate!` will itself generate a macro. This macro has the
/// name `export_{name}!` where `{name}` is the `snake_case` name of the `world`
/// that was having bindings generated.
///
...
/// # }
/// #
/// export_my_world!(MyComponent with_types_in self);
/// #
/// # fn main() {}
/// ```
///
...
/// # }
/// #
/// bindings::export_my_world!(MyComponent with_types_in bindings);
/// #
/// # fn main() {}
/// ```
///
...
///
///     // The generated `export_*!` macro, if any, will by default look for
///     // generated types adjacent to where the `export_*!` macro is invoked
