...
impl WorldGenerator for CSharp {
    ...
    fn finish(&mut self, resolve: &Resolve, id: WorldId, files: &mut Files) {
        ...
        if !&self.world_fragments.is_empty() {
            ...
            // TODO can this be shared with ExportsInterop?
        ...
        for (full_name, interface_type_and_fragments) in &self.interface_fragments {
            ...
            let body = format!(
                "// Generated by `wit-bindgen` {version}. DO NOT EDIT!
                {CSHARP_IMPORTS}
                using static {world_namespace}.exports.{name}World;
...
impl Bindgen for FunctionBindgen<'_, '_> {
    ...
    fn emit(
        ...
        match inst {
            ...
            Instruction::F64Load { offset } => todo!("F64Load"),
        ...
        match self.gen.direction {
            ...
            Direction::Export => {
                ...
                uwrite!(
                    self.src,
                    "
                    var {ptr} = returnArea.AddressOfReturnArea();
                    "
                );
