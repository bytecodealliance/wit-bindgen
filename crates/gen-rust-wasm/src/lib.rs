...
impl Generator for RustWasm {
    ...
    fn export(&mut self, iface: &Interface, func: &Function) {
        ...
        if func.is_async {
            self.src.push_str("};\n");
            self.src
                .push_str("witx_bindgen_rust::rt::execute(Box::pin(future));\n");
        }
        self.src.push_str("}\n");
    ...
    fn finish_one(&mut self, iface: &Interface, files: &mut Files) {
        ...
        for (name, trait_) in self.traits.iter() {
            if any_async {
                src.push_str("#[witx_bindgen_rust::async_trait(?Send)]\n");
            }
    ...
    fn emit(
        ...
        match inst {
            ...
            Instruction::CallWasmAsyncImport {
                ...
                self.push_str(") {\n");
                self.push_str("witx_bindgen_rust::rt::Sender::from_usize(sender).send((");
                ...
                self.push_str("let (rx, tx) = witx_bindgen_rust::rt::Oneshot::<(");
            ...
            Instruction::ReturnAsyncExport { .. } => {
                self.emit_cleanup();
                self.push_str(&format!(
                    "unsafe {{ witx_bindgen_rust::rt::async_export_done({}, {}); }}\n",
