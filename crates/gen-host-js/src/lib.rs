...
pub struct Opts {
    ...
    pub tla_compat: bool,
    /// Disable verification of Wasm data structures as a production optimization
    #[cfg_attr(feature = "clap", arg(long))]
    pub no_checks: bool,
}
...
impl ComponentGenerator for Js {
    ...
    fn finish_component(&mut self, name: &str, files: &mut Files) {
        ...
        if self.opts.tla_compat {
            ...
            if !self.opts.no_checks {
                uwriteln!(output, "_initialized = true;");
            }
            uwriteln!(output, "}})()");
        }
        ...
        match inst {
            ...
            Instruction::FlagsLift { flags, .. } => {
                ...
                if let Some(op) = operands.last() {
                    ...
                    // extraneous bits.
                    if flags.flags.len() % 32 != 0 && !self.gen.opts.no_checks {
