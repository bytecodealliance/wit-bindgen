...
use crate::exports::test::equal_types::blag::{Guest, Kind1, Kind3, Kind4, TStream, Tree, GuestInputStream};
...
impl Guest for Component {
    ...
    fn f(x: Kind1) -> Kind1 { blag::f(x) }
    fn g(_x: Kind3) -> Kind4 { todo!() }
    fn h(x: TStream) -> Tree {
        let x = blag::TStream { tree: x.tree, stream: None };
        blag::h(&x)
    }
}
...
impl HGuest for Component {
    fn f(x: Kind6) -> Kind5 { blah::f(x) }
    fn g(_x: Kind7)-> Kind4 { todo!() }
    fn h(x: TStream) -> CustomResult {
        let x = blah::TStream { tree: x.tree, stream: None };
        blah::h(&x)
    }
}
